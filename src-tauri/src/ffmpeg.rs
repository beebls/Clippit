...
pub async fn get_num_audio_tracks(file_name: String) -> Result<i32, String> {
  let command_output: std::process::Output = ffprobe_command().args(["-v", "error", "-select_streams", "a", "-show_entries", "stream=index", "-of", "csv=p=0", &file_name]).output().expect("Failed to execute the process");
...
pub async fn format_audio_track(track_num: i32, project_hash: String, volume: f32, start_time: f32, end_time: f32) -> Result<(), String> {
...
pub async fn encode_video_to_specific_filesize(project_hash: String, input_file: String, output_file: String, target_mebibytes: f32, encoder_type: String) -> Result<(), String> {
  let output_res: Option<PathBuf> = get_output_dir(project_hash).await;
  if output_res.is_none() {
    return Err("Can't get output dir".to_owned());
  }
  ...
  let duration_cmd: std::process::Output = ffprobe_command().args(["-v", "error", "-show_entries", "format=duration", "-of", "csv=p=0", &input_file]).output().expect("Failed to execute process");
  ...
  let audio_rate_cmd: std::process::Output = ffprobe_command().args(["-v", "error", "-select_streams", "a:0", "-show_entries", "stream=bit_rate", "-of", "csv=p=0", &input_file]).output().expect("Failed to execute process");
  ...
  let pass_two_output: std::process::Output = pass_two.output().expect("Failed to execute process");
